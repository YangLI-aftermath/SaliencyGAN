...
def load_batch_trainning_samples(train_images,
    ...
    np.random.seed()
    train_indexs = np.random.random_integers(low=0,high=n_train_samples-1,size=batch_size // 2) # stochastically choose the training samples
...
with graph.as_default():
    ...
    x_l = tf.placeholder(tf.float32, shape=[None, fix_size,fix_size,3],name="xl")   # labeled input
    x_u = tf.placeholder(tf.float32, shape=[None, fix_size,fix_size,3],name="xu")   # unlabeled input
    c = tf.placeholder(tf.float32, shape=[None,latent_varibale_length ],name="c")   # latent code
    tf.summary.histogram('c',c)

    c_label ,c_unlabel = tf.split(c,num_or_size_splits=2,axis=0) # split c(200) into c_label and c_unlabel evenly along batch dimension

    x = tf.concat([x_l,x_u],axis=0) # concatenate x_l and x_l along axis 0
    ...
    lr = tf.placeholder(tf.float32, shape=[],name='lr') # learning rate
    keep_prob = tf.placeholder(tf.float32,name='keep_prob') # keep_prob in the dropout layer
    ...
    x_hat_logits = Generator_DC_256(y_norm,
                                    c_label,
                                    base_n_channel=64,
                                    output_channel=3,
                                    keep_prob=keep_prob,
    ...
    x_cycle_logits = Generator_DC_256(y_out_final,
                                    c,
                                    base_n_channel=64,
                                    output_channel=3,
                                    keep_prob=keep_prob,
