...
def Generator_with_Four_Levels(x,keep_prob,layer,scope,fuse_channals,output_channel,growth_rate,compress_rate,is_training=True, reuse=True):
    ...
    with tf.variable_scope(scope, reuse=reuse): # push a name scope and a variable scope(only availiable on tensorflow v1)


        _,_,_,ch = x.get_shape().as_list() # get the channel of the input and return as a list
...
def Generator(x,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        x = batch_activ_conv(x,                 # BN + ReLU + CONV + DropOut ch 3->16
                             n_in_features=ch,
                             n_out_features=16,
                             keep_prob=keep_prob,
                             kernel_size=3,
        ...
        x = batch_activ_conv(x,                # BN + ReLU + CONV + DropOut ch 16->32
                             n_in_features=16,
                             n_out_features=32,
                             keep_prob=keep_prob,
                             kernel_size=3,
        ...
        return all_logits #Output Size = 6 [Batch] 256*256 @ 1 
...
def Generator_DC_with_code_256(x,c,base_n_channel,output_channel,keep_prob,scope,is_training=True, reuse=True):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        scale7 = batch_activ_conv_trans(scale6_fused,
                                        n_in_features= base_n_channel * 2 + base_n_channel // 8,
                                        n_out_features=base_n_channel,
                                        kernel_size=4,
                                        is_training=is_training,
                                        keep_prob=keep_prob,
        ...
        scale7_fused = tf.concat([scale7, scale7_map], axis=3)

        map = conv2d(scale7_fused,
                     n_in_features= base_n_channel + base_n_channel // 8,
                     n_out_features=output_channel,
                     kernel_size=3,
                     name='l7',
                     with_bias=True)
...
def Generator_DC_256(x,c,base_n_channel,output_channel,keep_prob,scope,is_training=True, reuse=True):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        scale7 = batch_activ_conv_trans(scale5_fused,
                                        n_in_features= base_n_channel * 2 + base_n_channel // 4,
                                        n_out_features=base_n_channel,
                                        kernel_size=4,
                                        is_training=is_training,
